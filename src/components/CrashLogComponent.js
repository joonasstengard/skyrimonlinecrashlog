...
function CrashLogComponent() {
  ...
  const [crashLogReportTitle, setCrashLogReportTitle] = useState(
    "Your Crash Log report will appear here"
  );
  ...
  const [crashLogReportHowToProceed, setCrashLogReportHowToProceed] =
    useState("");
  //============================================================================================================
  ...
  const generateReport = () => {
    ...
    setCrashLogReportTitle(
      "Generating report... This should take less than 5 seconds. "
    );
    ...
    if (
      firstLineOfCrashLog === undefined ||
      secondLineOfCrashLog === undefined ||
      thirdLineOfCrashLog === undefined
    ) {
      setCrashLogReportTitle(
        "It appears that you did not paste a full crash log. "
      );
      return;
    }
    ...
    if (firstLineOfCrashLog.includes("CLA")) {
      setCrashLogReportTitle(
        "It seems that you posted the output of another crash log analyzing tool, instead of a crash log. "
      );
      return;
    }
    if (
      thirdLineOfCrashLog.includes("NetScriptFramework") ||
      firstLineOfCrashLog.includes("NetScriptFramework")
    ) {
      setCrashLogReportTitle(
        "It seems that you posted a NetScriptFramework crash log. Unfortunately, NetScriptFramework crash logs are not supported by this tool. "
      );
      return;
    }
    ...
    for (const word of wordsToCheck) {
      if (!crashLogText.includes(word)) {
        setCrashLogReportTitle(
          "It appears that you did not paste a valid crash log. Check your input. "
        );
        return;
      }
    }
    ...
    const truncatedLog =
      startIndex !== -1 ? crashLogText.substring(startIndex) : crashLogText;
    const endIndexRegisters = truncatedLog.indexOf("STACK:");

    //registersLog is the name of the REGISTERS part of the log
    const registersLog =
      endIndexRegisters !== -1
        ? truncatedLog.substring(0, endIndexRegisters)
        : truncatedLog;

    //for checking the contents of probable call stack
    const probableCallStackStartIndex = crashLogText.indexOf(
      "PROBABLE CALL STACK:"
    );
    const truncatedProbableCallStack =
      probableCallStackStartIndex !== -1
        ? crashLogText.substring(probableCallStackStartIndex)
        : crashLogText;
    const probableCallStackEndIndex =
      truncatedProbableCallStack.indexOf("REGISTERS:");
    const probableCallStackLog =
      probableCallStackEndIndex !== -1
        ? truncatedProbableCallStack.substring(0, probableCallStackEndIndex)
        : truncatedProbableCallStack;
    ...
    for (const line of linesProbableCallStack) {
      ...
      if (line.includes("dyndolod")) {
        ...
        setCrashLogReportCause(
          "It seems likely that the crash was caused by DynDOLOD. "
        );
        return;
      }
    }
    ...
    const registersAndStackLog =
      endIndexStack !== -1
        ? truncatedLog.substring(0, endIndexStack)
        : truncatedLog;

    //searching for a reason for the crash
    var reasonForCrash = handleRegistersLog(registersLog, registersAndStackLog);

    var espThatCausedCrash =
      searchForPluginFromRegistersAndStack(registersAndStackLog);
    ...
    if (modifiedByLine !== "Not found. ") {
      ...
      setCrashLogReportModifiedBy(
        "Crash was likely caused by one of the following mods: " +
          modifiedByLine
      );
      setCrashLogReportPlugin(
        "Probably one of them accessing or clashing with this mod: " +
          espThatCausedCrash +
          ". "
      );
    } else {
      ...
      if (espThatCausedCrash !== "Not found. ") {
        setCrashLogReportPlugin(
          "The following plugin (mod) might have been involved in the crash: " +
            espThatCausedCrash +
            ". "
        );
      } else {
    ...
    if (formIDThatCausedCrash !== "Not found. ") {
      setCrashLogReportFormID(
        "The following record might have been involved in the crash: " +
          formIDThatCausedCrash +
          ". "
      );
    } else {
      setCrashLogReportFormID(
    ...
    if (modifiedByLine !== "Not found. ") {
      howToProceedInstructions =
        "To fix the issue, you can try disabling one or more of the mods mentioned above. If the crash still happens, try again with a new log. ";
      if (formIDThatCausedCrash !== "Not found. ") {
        howToProceedInstructions =
          howToProceedInstructions +
  ...
  function handleRegistersLog(registersLog, registersAndStackLog) {
    ...
    if (lowercaseRegistersLog.includes("facegen")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("Name:")) {
          ...
          return (
            "Probably a FaceGen issue. Could be missing or corrupt FaceGen data or some other NPC face related problem. Possibly related to: " +
            line.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes("textures")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("textures")) {
          // If found, return the line
          return (
            "The following texture is seemingly causing an issue: " +
            line.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes(".nif")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes(".nif")) {
          // If found, return the line
          return (
            "The following mesh is seemingly part of the issue: " + line.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes("bslightingshadermaterialenvmap")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("Full Name:")) {
          ...
          return (
            "Seems like an issue with a BSLightingShaderMaterialEnvmap. Possibly related to the following: " +
            lineWithoutPrefix.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes("bslightingshader")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("Full Name:")) {
          ...
          var lineWithoutPrefix = line.replace("Full Name:", "");
          return (
            "Possibly an issue with a BSLightingShader. Possibly related to the following: " +
            lineWithoutPrefix.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes("ninode")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("name:")) {
          // If found, return the line
          return (
            "Looks like it might be a NiNode issue, possibly related to: " +
            line.trim()
          );
        }
      }
    ...
    if (
      lowercaseRegistersLog.includes("hka") ||
      lowercaseRegistersLog.includes("hkb")
    ) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("name:")) {
          // If found, return the line
          return (
            "It might be an animation issue, possibly related to: " +
            line.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes("tesobjectarmo")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("name:")) {
          // If found, return the line
          return (
            "Probably an issue related to some armor, clothing or jewelry piece, possibly related to this: " +
            line.trim()
          );
        }
      }
    ...
    if (
      lowercaseRegistersLog.includes("hdtsmp") ||
      lowercaseRegistersLog.includes("hdt-smp")
    ) {
    ...
    if (lowercaseRegistersLog.includes("armor")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("armor")) {
          // If found, return the line
          return (
            "Can't give a simple answer for this log, but it seems like it could be related to: " +
            line.trim()
          );
        }
      }
    ...
    if (lowercaseRegistersLog.includes("hrtfxapoeffect")) {
      ...
      for (const line of lines) {
        ...
        if (line.includes("Audio")) {
          //If found, return the line
          return (
            "There is very little information available online about HrtfXapoEffect, but it could indicate an audio related issue. Possible related to the following line in your crash log: " +
            line.trim()
          );
        }
      }
  ...
  return (
    <div className="container flex flex-col items-center justify-center mx-auto mt-5">
      <textarea
        ...
        className="w-full lg:w-3/5 p-4 mt-2 rounded-md font-semibold hover:border-gray-700 bg-black bg-opacity-70 h-64 resize-y fade-effect"
        id="crashLogInput"
        value={crashLogText}
        onChange={handleTextareaChange}
      />
      <button
        className="mt-5 p-3 rounded-md hover:border-gray-700 bg-black bg-opacity-75"
      ...
      <div className="w-full lg:w-3/5 pt-2 pl-4 pr-4 pb-4 mt-3 rounded-md font-semibold bg-black bg-opacity-70 resize-y fade-effect">
